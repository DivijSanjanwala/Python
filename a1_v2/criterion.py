    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for i in range(len(answers)):
            for j in range(i + 1, len(answers)):
                ...
                if answers[i].is_valid(question) and \
                        answers[j].is_valid(question):
                    count += question.get_similarity(answers[i], answers[j])
                    num_combinations += 1

                else:
                    raise InvalidAnswerError

        return count / num_combinations
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for answer1 in answers:
            if not answer1.is_valid(question):
                raise InvalidAnswerError

            count = 0
            for answer2 in answers:
                if not answer2.is_valid(question):
                    raise InvalidAnswerError
