...
class MultipleChoiceQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        question = f"Question: {self.text}"

        # Initializing the answer options
        answers = "Pick one of the following options:"
        for i in range(len(self._options)):
            answers += f"\n{i}) {self._options[i]}"

        # Return question and answer as a single string
        return question + "\n" + answers
...
class NumericQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        """
        question = f"Question: {self.text}"
        answer = f"Pick an integer between {self._min} and {self._max} " + \
                 f"inclusive."
        return question + "\n" + answer
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        diff_max_min = self._max - self._min

        return 1.0 - (diff / diff_max_min)
...
class YesNoQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        """
        question = f"Question: {self.text}"
        answer = "Answer as True (yes) or False (no)."
...
class CheckboxQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        # Initializing the answer options
        answers = "Pick one (or more) of the following options:"
        for i in range(len(self._options)):
            answers += f"\n{i}) {self._options[i]}"
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        # Keep track of all answer.content in a list so that duplication leads
        # to returning False
        existing = []

        for content in answer.content:
            if content not in self._options or content in existing:
                return False
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        all_str = len(set1.union(set2))

        return common / all_str
...
class Survey:
    ...
    def __init__(self, questions: List[Question]) -> None:
        ...
        """
        # Construct the self._questions dictionary
        self._questions = {}
        for question in questions:
            self._questions[question.id] = question

        # Construct rest of the attributes
        self._criteria = {}
        self._weights = {}
    ...
    def __str__(self) -> str:
        ...
        # Include everything till the extra "\n\n" added at the end
        return result[0:-2]
    ...
    def score_students(self, students: List[Student]) -> float:
        ...
        for id_ in self._questions:
            # Create references to required objects associated with this id_
            question = self._questions[id_]
            criterion = self._get_criterion(question)
            weight = self._get_weight(question)

            # Make a list of answers by all students to this specific question
            answers = []
            for student in students:
                answers.append(student.get_answer(question))

            # While adding to score, check for InvalidAnswerError
            try:
                score += weight * criterion.score_answers(question, answers)
